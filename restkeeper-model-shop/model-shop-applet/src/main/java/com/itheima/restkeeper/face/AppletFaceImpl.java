 ...
 */
@Slf4j
@DubboService(version = "${dubbo.application.version}", timeout = 5000,loadbalance = "",
    methods = {
    ...
    public OrderVo placeOrder(Long orderNo) throws ProjectException {
        ...
        try {
            if (lock.tryLock(
                ...
                orderVoResult.setOrderItemVoTemporaryList(orderItemVoTemporaryList);
                //5、购物车订单项不为空才合并
                if (!EmptyUtil.isNullOrEmpty(orderItemVoTemporaryList)) {
                    //6、求交集:购物车订单项与核算订单项合并
                    ...
                    if (!EmptyUtil.isNullOrEmpty(listDifference)) {
                        ...
                        if (!flag) {
                            projectException =
                                    new ProjectException(OrderItemEnum.UPDATE_ORDERITEM_FAIL);
                        }
                    }
                    //8、计算订单金额
